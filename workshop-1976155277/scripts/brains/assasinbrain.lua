...
local AssasinBrain = Class(Brain, function(self, inst)
...
local function HideRunAway(inst, target)
    return inst.hide
end

function AssasinBrain:OnStart()
    local root = PriorityNode(
    {			--RunAway(self.inst, "player", 4, 6, function(target) return HideRunAway(self.inst, target) end ),
	
				WhileNode( function() return self.inst.components.health:GetPercent() < 0.5 and self.inst.components.combat.target == nil or not self.inst.components.combat:InCooldown() end, "AttackMomentarily", ChaseAndAttack(self.inst, 8,40)),			
			WhileNode( function() return self.inst.components.health:GetPercent() < 0.5 and self.inst.components.combat.target and self.inst.components.combat:InCooldown() end, "Dodge", RunAway(self.inst, function() return self.inst.components.combat.target end, 5, 6)),
 ...
 Wander(self.inst, function() return Vector3(self.inst.Transform:GetWorldPosition()) end, MAX_FOLLOW_DIST- 1, {minwalktime=3, randwalktime=3, minwaittime=6, randwaittime=3}),
    }, 0)
