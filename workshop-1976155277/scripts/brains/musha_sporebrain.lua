...
function Mushasporebrain:OnStart()
	...
	PriorityNode(
	{
		...
		Wander(self.inst, function() if self.inst.components.follower.leader then return Vector3(self.inst.components.follower.leader.Transform:GetWorldPosition()) end end, MAX_FOLLOW_DIST- 1, {minwalktime=1, randwalktime=1, minwaittime=1, randwaittime=5.0})	
	}, 1)
