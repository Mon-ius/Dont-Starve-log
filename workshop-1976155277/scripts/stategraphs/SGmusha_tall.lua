...
-------------------------------------------------------------------
    ...
    State{
        ...
        timeline = 
        {
            ...
            --TimeEvent(1*FRAMES, function(inst) inst.Transform:SetScale(.7, .7, .7) end),
            TimeEvent(1*FRAMES, function(inst) inst.SoundEmitter:PlaySound("dontstarve/creatures/teenbird/leg_sproing") end),
            --TimeEvent(1*FRAMES, function(inst) inst.Transform:SetScale(.75, .75, .75) end),
        },
        events=
        {
            EventHandler("animover", function(inst)
                inst.userfunctions.SpawnAdult2(inst)
            end),
        },
    },
...
return StateGraph("musha_tall", states, events, "idle", actionhandlers)
