...
-------------------------------------------------------------------
    ...
    State{
        ...
        timeline = 
        {
            ...
            --TimeEvent(1*FRAMES, function(inst) inst.Transform:SetScale(.8, .8, .8) end),
            TimeEvent(1*FRAMES, function(inst) inst.SoundEmitter:PlaySound("dontstarve/creatures/teenbird/leg_sproing") end),
            --TimeEvent(1*FRAMES, function(inst) inst.Transform:SetScale(.85, .85, .85) end),
        },
        events=
        {
            EventHandler("animover", function(inst)
                inst.userfunctions.SpawnAdult3(inst)
            end),
        },
    },
...
return StateGraph("musha_tall2", states, events, "idle", actionhandlers)
