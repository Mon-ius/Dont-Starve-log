...
local function OnAttackFn(inst, data)
    ...
    if not inst.components.health:IsDead() and not (vehicle and vehicle.components.boathealth and vehicle.components.boathealth:IsDead()) then
        ...
        if not vehicle.components.drivable:CanDoHit() then return end
...
local states=
{
    ...
    State{
	 ...
	 State{
		...
		inst.components.talker:Say(GetString(inst.prefab, "ANNOUNCE_REFUSE_BOAT1"))
		elseif math.random() < 0.5 then  
		inst.components.talker:Say(GetString(inst.prefab, "ANNOUNCE_REFUSE_BOAT2"))
		elseif math.random() < 0.8 then  
		inst.components.talker:Say(GetString(inst.prefab, "ANNOUNCE_REFUSE_BOAT3"))
		else
		inst.components.talker:Say(GetString(inst.prefab, "ANNOUNCE_HATEFOOD"))
			end	
        end,
        ...
        events=
        {
            EventHandler("animover", function(inst) if inst.AnimState:AnimDone() then inst.sg:GoToState("idle") inst.keep_check = false end end ),
        },
        events=
        {
            EventHandler("animqueueover", function(inst) inst.sg:GoToState("idle") inst.keep_check = false end),
        },
        ...
        onenter = function(inst, noanim)
			...
			for k = 1, math.random(2, 3) do
                ...
                if k == 1 then
                    ...
                    inst.AnimState:PlayAnimation("mime2", false)
