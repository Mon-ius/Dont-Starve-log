...
if SaveGameIndex:GetCurrentMode() == "porkland" or SaveGameIndex:GetCurrentMode() == "shipwrecked" or GetPlayer().components.poisonable or GetPlayer().components.moisture then
	self.paused = false
    self.pause_time = 0
...
function Pickable:SetUp(product, regen, number, stock)
...
function Pickable:CanBePicked()
	if self.canbepickedfn then
		if not self.canbepickedfn(self.inst) then
			return false
		end
	end
    return self.canbepicked
...
-- Does the same as the above, except it doesn't spawn loot
-- It's used by the shearable in grass.lua
function Pickable:SimulatePick(picker)
if SaveGameIndex:IsModePorkland() or SaveGameIndex:GetCurrentMode() == "porkland" then
	if self.canbepicked and self.caninteractwith then
        ...
        if self.onpickedfn then
            self.onpickedfn(self.inst, picker, nil)
        end
