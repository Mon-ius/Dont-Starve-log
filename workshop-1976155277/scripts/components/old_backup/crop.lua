...
function Crop:DoGrow(dt)
    if not self.withered then
        ...
        if season:GetTemperature() < TUNING.MIN_CROP_GROW_TEMP then
			...
			elseif GetPlayer().components.moisture and not GetPlayer().components.poisonable then	
				if season:IsSpring() then
                weather_rate = weather_rate + (TUNING.SPRING_GROWTH_MODIFIER/3)
				end
			elseif SaveGameIndex:IsModeShipwrecked() or GetPlayer().components.poisonable then	
				if season:IsGreenSeason() then
                weather_rate = weather_rate + (TUNING.SPRING_GROWTH_MODIFIER/3)
				end	
...
if not GetPlayer().components.moisture then
