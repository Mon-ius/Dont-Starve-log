...
function Breeder:CollectSceneActions(doer, actions)
    if self.volume > 0 and doer.components.inventory and not doer:HasTag("yamcheb") and not doer:HasTag("critter") then
        table.insert(actions, ACTIONS.HARVEST)
		
    elseif self.volume > 0 and doer.components.container and (doer:HasTag("yamcheb") or doer:HasTag("critter")) then
        table.insert(actions, ACTIONS.HARVEST)
    end
end
...
function Breeder:Harvest(harvester)
    ...
    self.harvestable = false
    self.harvested = true
    if harvester.components.inventory and not harvester:HasTag("yamcheb") and not harvester:HasTag("critter") then
        local product = SpawnPrefab(self.product)
        harvester.components.inventory:GiveItem(product)
	elseif harvester.components.container and (harvester:HasTag("yamcheb") or harvester:HasTag("critter")) then
        local product = SpawnPrefab(self.product)
