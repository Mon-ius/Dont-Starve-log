...
local function RetargetFn(inst)
    ...
    nil,
      {"musha","player","wall","houndmound","structure","companion","yamche","arongb","pig","bee","rocky","webber","bird","koalefant","beefalo","companion","moondrake","moondrake2","butterfly","smashable","veggie","prey"})
	end
end 	
...
local ents = TheSim:FindEntities(x,y,z, 30, {"dall"})
for k,v in pairs(ents) do
if v:HasTag("dall") and not inst.components.follower.leader then
...
local function fn()
    ...
    inst.AnimState:SetBuild("moondrake_build")
...
return Prefab("moonnutdrake", fn, assets, prefabs)
