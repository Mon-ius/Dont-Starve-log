...
--[[if math.random() < 0.7 and target.components.health and not target.components.health:IsDead() and target:HasTag("bird") then
	...
	GetWorld():DoTaskInTime(0.3, function() 
	if not target.components.health:IsDead() then
	target.sg:GoToState("flyaway") target.components.health:SetInvincible(false)
...
--[[if math.random() < 0.5 and target.components.health and not target.components.health:IsDead() and (target:HasTag("rabbit") or target:HasTag("butterfly")) then
	target.components.health:SetInvincible(true)
	if target:HasTag("rabbit") then	
	target:DoTaskInTime(0.3, function() target:PushEvent("gohome") end)
	end
	GetWorld():DoTaskInTime(0.5, function() 
	if not target.components.health:IsDead() then
...
if target:HasTag("bird") and target.components.health and not target.components.health:IsDead() then	
target:DoTaskInTime(0.3, function() target:PushEvent("flyaway") end)
end
--end
end
...
--check arrow
	local item = attacker.components.inventory:FindItem(function(item) 	
	return item.prefab == "arrowm" end )
			if item then
...
SpawnPrefab("splash").Transform:SetPosition(inst:GetPosition():Get())
	...
	-----
	if target.components.locomotor and target.components.health and not target.components.health:IsDead() and not target:HasTag("shadowcreature") and not target:HasTag("structure") and not inst.boost then
		...
		poison.Transform:SetScale(0.5,0.8,0.5)
		...
		GetWorld():DoPeriodicTask(3, function() 
		if target:HasTag("slow_poison") and not target.components.health:IsDead() then 
				target.components.health:DoDelta(-4)
			if not target:HasTag("giant") and not target:HasTag("epic") and not target:HasTag("wet") and not target:HasTag("ghost") then			
				...
				--target.SoundEmitter:PlaySound("dontstarve/wilson/attack_weapon")
		elseif poison and target.components.health:IsDead() then
				poison:Remove()
		end	end)
		GetWorld():DoTaskInTime(9, function() target.components.locomotor.groundspeedmultiplier = 1 target:RemoveTag("slow_poison") poison:Remove() end)
	...
	elseif math.random() < poisone_2 and not inst.boost and inst.level >=250 and inst.level <1400 and not target:HasTag("slow_poison") then
		...
		GetWorld():DoTaskInTime(12, function() target.components.locomotor.groundspeedmultiplier = 1 target:RemoveTag("slow_poison") poison:Remove() end)
	...
	elseif math.random() < poisone_3 and not inst.boost and inst.level >=1400 and not target:HasTag("slow_poison") then
		...
		GetWorld():DoTaskInTime(15, function() target.components.locomotor.groundspeedmultiplier = 1 target:RemoveTag("slow_poison") poison:Remove() end)
		end
...
local function onequip(inst, owner)	
 	...
 	local item = owner.components.inventory:FindItem(function(item) 	
	return item.prefab == "arrowm" end )
			if item then
				inst.arrow_ready = true
				else
...
-- check arrow	
	...
	return item.prefab == "arrowm" end )
			if item then
				inst.arrow_ready = true
				--owner.components.talker:Say("Bow !")
				else
				inst.arrow_ready = false	
				inst:RemoveTag("bowm")
