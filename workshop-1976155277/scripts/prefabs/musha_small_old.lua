...
if inst.sleep_on and not inst.hat and not inst.house then
local random2 = 0.2
...
if owner ~= nil and not owner:HasTag("musha") and owner.components.inventory then
	if inst.components.burnable and not inst.components.burnable:IsBurning() and not inst.escape_fire then
...
if v:HasTag("sailing") or GetPlayer():HasTag("aquatic") and inst.components.follower and inst.components.follower.leader and not inst.sleep_on then
...
if not v:HasTag("sailing") then
inst.yamche_wing = false	
inst.splash_on = false 

end

--[[if inst.house or inst.hat then
	if inst.components.burnable and inst.components.burnable:IsBurning() then
	inst.components.burnable:Extinguish() 
	end
end	]]
...
local turf = GetWorld().Map:GetTileAtPoint(yamchaep.x, yamchaep.y, yamchaep.z)
if turf ~= GROUND.FOREST and turf ~= GROUND.GRASS and turf ~= GROUND.SAVANNA and turf ~= GROUND.DECIDUOUS and turf ~= GROUND.JUNGLE and turf ~= GROUND.MEADOW and turf ~= GROUND.RAINFOREST and turf ~= GROUND.DEEPRAINFOREST and turf ~= GROUND.LAWN and turf ~= GROUND.GASJUNGLE and turf ~= GROUND.FIELDS and turf ~= GROUND.FUNGUS and turf ~= GROUND.FUNGUSRED and turf ~= GROUND.FUNGUSGREEN and turf ~= GROUND.DIRT and turf ~= GROUND.MUD and turf ~= GROUND.DESERT_DIRT and turf ~= GROUND.BEACH and turf ~= GROUND.SWAMP and turf ~= GROUND.MAGMAFIELD and turf ~= GROUND.ASH then
...
--------------------
	...
	inst.yamche_wing = inst:DoPeriodicTask(0,yamche_wing)
	inst:DoPeriodicTask(0.5, function() if inst.yamche_wing then splash_on(inst) end end)
    ...
    inst.Peaceful = inst:DoPeriodicTask(0.1, Peace)   
...
inst:ListenForEvent("hungerdelta", on_wakeup)
	...
	inst.growup_check = inst:DoPeriodicTask(0, function() if inst.grow_up_yamchae_ready then growup_check(inst) end end)
...
local leader = inst.components.follower.leader
if inst.components.follower.leader == nil and inst.components.staycommand and not inst.hat and not inst.house then
inst.sleep_on = true
inst.stay = true
inst.components.staycommand:SetStaying(true)
inst.components.staycommand:RememberStayPos("stayspot", Point(inst.Transform:GetWorldPosition()))
