...
local function retargetfn(inst)
    ...
    return not inst.sg:HasStateTag("lure")
        and FindEntity(inst, 12, function(guy) 
        if guy.components.combat and guy.components.health and not guy.components.health:IsDead() then
            return ( guy:HasTag("character") or guy:HasTag("animal")) and not 
...
local function shouldKeepTarget(inst, target)
	...
	if not inst.sg:HasStateTag("lure") and target and target:IsValid() and target.components.health and not target.components.health:IsDead() then
		if home then
...
local function displaynamefn(inst)
    if inst.sg:HasStateTag("lure") then
        return STRINGS.NAMES.GWORM_PLANT
    elseif inst.sg:HasStateTag("dirt") then
        return STRINGS.NAMES.GWORM_DIRT
    end
    return STRINGS.NAMES.GWORM 
end

function lure_plan(inst)
local bait = FindEntity(inst, 20, function(target) 
                return target:HasTag("player") and not inst.components.combat.target == target:HasTag("player") end)
if bait then
...
local function fn()
    ...
    MakeCharacterPhysics(inst, 800, .5)
...
inst.components.health:SetMaxHealth(600)
    ...
    --inst.components.health:SetMaxHealth(600) --900
    ...
    inst.components.lighttweener:StartTween(light, 0, 0.2, 0.1, {1,1,1}, 0, function(inst, light) if light then light:Enable(false) end end)

    inst:AddComponent("knownlocations")
    inst:AddComponent("inventory")
    inst:AddComponent("inspectable")
    inst:AddComponent("lootdropper")
    inst.components.lootdropper:SetLoot({"monstermeat", "monstermeat", "green_fruit", "green_fruit" })
	...
	inst.lure_plan = inst:DoPeriodicTask(10, lure_plan)
    inst.lastluretime = 0
...
return Prefab( "greenworm", fn, assets, prefabs) 
