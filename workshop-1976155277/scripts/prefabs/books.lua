...
function birdstestfn(inst, reader)
...
function volcanofn(inst, reader)
    local delay = 0.0
...
function MakeBook(name, usefn, bookuses, testfn )
	...
	if SaveGameIndex:IsModePorkland() or SaveGameIndex:IsModeShipwrecked() then
        MakeInventoryFloatable(inst, name.."_water", name)
    end   
        -----------------------------------
...
return MakeBook("book_sleep", sleepfn, 5),
       ...
       MakeBook("book_birds", birdsfn, 3, birdstestfn),
       MakeBook("book_tentacles", tentaclesfn, 5), 
       MakeBook("book_meteor", volcanofn, 5)
