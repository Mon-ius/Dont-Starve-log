				...
				if layout.areas[current_prefab] ~= nil then
					...
					if area_contents ~= nil then
						for i,r_prefab in ipairs(area_contents) do
							...
							local properties = deepcopy(current_prefab_data.properties)
...
--		end
			...
			for column = 1, size do
				...
				if layout.ground[clmn][rw] ~= 0 then	
					if position ~= nil then
						...
						WorldSim:SetTile(position[1] + column - 1, size+position[2] - row, layout.ground_types[layout.ground[rw][clmn]], 1)
					else
	...
	layout.border = layout.border or 0

	-- reserve the area
	local rcx = 0
	local rcy = 0
	if position == nil then
		rcx, rcy = WorldSim:ReserveSpace(node_id, size, layout.start_mask, layout.fill_mask, layout.layout_position, layout.border, tiles)
	else
		rcx = position[1]
		rcy = position[2]
	end
